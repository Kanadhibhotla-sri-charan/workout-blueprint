...
## Final implementation pass (PHASE-5-FINAL-UI-AND-IMPLEMENTATION-SPEC.md)

A concrete final pass against the spec's own two categories — functional refinements and a fully prescriptive UI direction — implemented against the existing Phase 5 architecture, per the spec's own instruction not to reinterpret the product structure or open a Phase 6.

### A. Functional refinements

- **P5-F1 (bug fix):** `BuildMusclePackagePage.tsx`'s "Why this exercise?" `<summary>` was rendering `resolvedExercise.intensityTechniqueContext` instead of the package's own authored `entry.contribution` — confirmed via `grep` before touching anything, then fixed by splitting the exercise card into three independently-expandable `<details>` blocks ("Why this exercise?" / "Intensity technique" / "Progression"), each now showing the field it's actually named for.
- **P5-F2/F3 (verify, don't rebuild):** an audit probe (deleted after use) confirmed `packageEngine.ts`'s existing reuse of `buildProgramming()` already produces a genuinely varied, contextual intensity-technique distribution across all 22 packages (none=12, rest-pause=15, drop-set=15, myo-reps=33) and correctly withholds techniques from heavy/high-skill compounds (`back-squat`, `romanian-deadlift`, `bulgarian-split-squat-*`, `dip-triceps-biased` all resolve to `none`). No engine change was needed — only the P5-F1 display fix and giving the technique its own visible section instead of leaving it folded into "Why this exercise?".
- **P5-F4 (audit, don't force):** most packages already resolve to 2-3+ distinct sets/reps combinations across their exercises (a consequence of the original Phase 5 profile-driven `reps` design). The few packages with identical prescriptions across every exercise (forearms, calves) are that way because their exercises genuinely resolve to the same Programming Profile — a correct outcome to leave alone, not a defect to force apart, per the spec's own instruction against creating arbitrary differences.
- **P5-F5 (data-driven Efficient vs. Complete):** added `comparePackageLevels(efficient, complete)` to `packageEngine.ts` — computes real `addedExercises` (Complete-only exercise ids/names) and `addedTargetNames` (targets Complete covers that Efficient doesn't already cover) directly from resolved package data. Verified it never over-claims: for chest, `addedTargetNames` is `[]` because Efficient's cable-fly already covers all 3 pec targets by itself, even though Complete adds two more exercises. Drives the new "What Complete adds" bullet list on the level-selector cards — no hand-written per-muscle marketing copy.
- **P5-F6 (volume/redundancy sanity check, all 22 packages):** cross-checked every package's weekly volume against the app's only two authoritative volume bands (`global-principles.yaml`'s `practical_range_sets` `[10,20]` and `higher_recovery_dependent_sets` `[20,26]`). Found one real issue: `calves-efficient`/`calves-complete` and `core-efficient`/`core-complete` were authored at `sessions_per_week: 3` on the strength of a rationale sentence claiming calves/core are "commonly trained more often" — an unsupported claim, since `global-principles.yaml` defines exactly one universal frequency range and nothing elsewhere in the dataset corroborates a per-muscle exception. At 3×/week, `core-efficient` hit 24 weekly sets (above the practical range) and `core-complete` hit 30 (above even the higher-recovery-dependent band). Fixed by changing all four packages to `sessions_per_week: 2` and removing the unsupported sentence from their `rationale` fields. Every package now sits in a defensible band: Efficient always ≤16 weekly sets; Complete either within `[10,20]` or in the real `[20,26]` band the app already defines. Added a computed (never authored) `isHighVolume` flag (`weeklyDirectSets > practical_range_sets[1]`) to `ResolvedPackage`, surfaced as a "High-volume" badge using a new `--warning` token (not `--danger` — high volume is informational, not an error).

### B. Product/UI refinement

Rewrote both Build pages (`BuildMuscleIndexPage.tsx`, `BuildMusclePackagePage.tsx`) and extended `index.css` to match the spec's prescriptive layout/hierarchy sections, all still sourced from real package/engine data — no hardcoded per-muscle text or numbers anywhere in the new markup:

- Added a semantic design-token alias layer (`--bg-page`, `--bg-surface`, `--bg-surface-elevated`, `--border-subtle`, `--text-primary`, `--text-secondary`, `--accent-primary`, `--accent-secondary`, `--success`, `--warning`, `--radius-sm/-md/-lg`) as additive aliases onto the existing physical tokens — never a renaming, so nothing outside Build's own CSS block could regress. Applied via a scoped substitution restricted to the `/* Build the Muscle */` CSS block onward, confirmed via `grep -c` to leave Explore/Decide/Home untouched.
- Muscle overview page now follows the spec's exact hierarchy: back-nav → muscle identity → visual objective → 3 hero metric tiles (weekly sets / frequency / coverage %, all real) → coverage visualization (real per-target booleans, no fabricated percentages) → Efficient/Complete selector as full comparison cards (replacing the old tab UI) → selected-package summary with a real volume bar (band = global practical range, fill = actual weekly sets) → exercise sequence → weekly plan panel → package rationale.
- Exercise cards: each of the three sections (Why this exercise? / Intensity technique / Progression) is now independently expandable, and role badges show text, never a bare color swatch or an exposed `UNSPECIFIED`.
- Weekly plan panel states "every session is the same combination, run N times a week" rather than duplicating identical session lists — the data model has no per-session differentiation, and the spec explicitly permits stating that rather than faking variation.
- Added a global `@media (prefers-reduced-motion: reduce)` rule — found missing from the entire app (a pre-existing gap from all prior phases, not introduced by this pass) and required by spec §33.
- `buildExerciseProgressionNote(reps)` added to `packageEngine.ts`: parses an exercise's own resolved rep range (e.g. `"6-12"`) and returns a parametrized restatement of the app's existing double-progression model (`global-principles.yaml`'s `progression.explanation`) — same philosophy, real numbers substituted in, not a new progression model.

### Verification

- `packageEngine.test.ts` grew from 6 to 11 tests: `comparePackageLevels` (added-exercise set matches reality for all 11 groups and is always non-empty; chest's `addedTargetNames` is `[]`), `buildExerciseProgressionNote` (parametrizes correctly; falls back to the global explanation on unparseable input), `isHighVolume` (matches the real threshold comparison for all 22 packages; always `false` for Efficient).
- Full regression: `npx tsc -b --force`, `oxlint`, `npm run validate-data`, and the full Vitest suite (**158/158 passing**) all green.
- Manual adversarial content check (spec §38): extracted live-rendered exercise names/rationale text for all 6 named example muscle groups (chest, calves, back, shoulders, arms, core) directly from the running app, not just the authored YAML — all pass.
