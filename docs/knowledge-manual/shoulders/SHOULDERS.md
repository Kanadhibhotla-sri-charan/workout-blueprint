# Shoulders — Draft Knowledge Module
...
- **Why this exists:** The main loadable vertical press, useful when a user wants overhead pressing strength rather than only shoulder size.
...
- **Why this exists:** A stable vertical press that lets the user push closer to failure without managing balance or bar path.
- **Primary targets:** Anterior deltoids; triceps contribute.
- **Movement:** Vertical press.
...
- **Why this exists:** The most available direct side-delt option, and the main way to add shoulder width when pressing already covers the front delts.
...
- **Why this exists:** A side-delt option whose resistance stays meaningful lower in the range than a dumbbell raise, which changes where the movement feels hardest.
- **Primary targets:** Lateral deltoids.
- **Movement:** Shoulder abduction.
...
- **Why this exists:** Isolates shoulder external rotation, which is difficult to load meaningfully inside larger pressing and pulling movements.
...
- **Why this exists:** A low-equipment way to train scapular protraction, which most pressing and pulling does not load directly.
