...
- **Why this exists:** A bodyweight vertical pull that requires little equipment and scales through added load, tempo, or assistance. The overhand grip leans on the lats and grip strength more than the chin-up does.
...
- **Why this exists:** The same bodyweight vertical pull as the pull-up, but the underhand grip brings the biceps in more directly, which often lets a user complete more reps or add more load at the same relative lat effort.
...
- **Why this exists:** A vertical pull with adjustable load, which makes it easier to control repetition ranges and progression than bodyweight pulling. The wide pronated grip is the default most people mean by "lat pulldown."
...
- **Why this exists:** A supinated, closer-than-wide grip that brings the biceps into the pull more, similar to how a chin-up relates to a pull-up, while keeping the adjustable load of a cable stack.
...
- **Why this exists:** A parallel or angled grip that avoids the shoulder's end-range external rotation a wide pronated grip asks for, which many find is the kindest grip on the shoulders and elbows.
...
- **Why this exists:** A horizontal pull where the bench removes most of the lower-back and balance demand, so effort goes into the pull itself.
...
- **Why this exists:** The reference free-weight horizontal pull, loadable with small increments and useful when a user wants one row that also demands trunk and hip control. The overhand grip is the default most people mean by "barbell row," and it flares the elbows wider than an underhand grip, biasing the upper back and rear delts somewhat more.
...
### Reverse-Grip (Underhand) Barbell Row

- **Why this exists:** The same free-weight row with the grip flipped underhand, which pulls the elbows in closer and brings the lats and biceps into the pull more, the same relationship a chin-up has to a pull-up.
...
- **Overlaps:** Rack pull below, and Romanian deadlift, recorded in the hips and hamstrings modules; each starts from a different position or range and loads the pull differently. The sumo deadlift, recorded in the quads module, is a wide-stance version of this same floor pull — its primary emphasis shifts enough toward the quads and adductors that it's recorded there rather than duplicated here. A Smith machine is not a practical substitute for either stance — the floor-level starting position and the coordinated hip-and-knee path a full pull needs don't fit a fixed vertical rail.
- **Status:** Draft.
...
- **Why this exists:** A unilateral horizontal pull that lets each side work through its own range and exposes side-to-side differences.
...
- **Why this exists:** A lat-focused option with the elbow mostly fixed, which may be useful when elbow flexors fatigue before the lats in pulldowns and rows.
...
- **Why this exists:** Direct upper-trap loading for users who want that area trained beyond what rows and pulls provide.
