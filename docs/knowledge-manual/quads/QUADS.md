...
- **Why this exists:** The main loadable knee-dominant pattern, and the reference point most other quad work is compared against.
...
- **Why this exists:** Allows high loads with minimal balance and trunk demand, and is easy to run close to failure alone.
...
- **Why this exists:** The only common movement that loads knee extension without the hip, so it can add quad work at low systemic cost.
...
- **Why this exists:** Loads knee extension with the hip extended, so the rectus femoris works in a lengthened position, and it needs almost no equipment.
